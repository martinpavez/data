...
print(id_experiments[14:])
k=0
for id in id_experiments[16:]:
    t1 = time.time()
    k+=1
    print("Executing",id, "iter", k)
    data = {1: pd.read_parquet(f"data/climate_features/{region}/predictor_{id}_year_v2.parquet")}
    ...
    xgb_model = XGBCustomObjective(
        objective_func=sera_objective(piecewise_linear_phi_np(bounds_v1, initial_weight=i_w)),
        n_estimators=15,
        learning_rate=0.1
    )
    ...
    assert len(regressors) == len(name_regressors)
    experiment_1 = PredictionExperiment(data, indices_of_interest, regressors, name_regressors, len_prediction, id, whole_year=True, loss_fn=SERA(bounds=bounds_v1,T=100, initial_weight=i_w, fn="piecewise2"))
    ...
    experiment_1.save_results(f"data/sera_results_year_v2/{region}_results/results.csv")
    del experiment_1
